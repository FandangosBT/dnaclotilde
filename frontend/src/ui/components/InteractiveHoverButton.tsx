...
export default function InteractiveHoverButton({ className = '', children, ...rest }: InteractiveHoverButtonProps) {
  ...
  return (
    <button
      ...
      className={[
        // layout base
        'group relative inline-flex items-center justify-center overflow-hidden rounded-lg px-3 py-2 font-coder',
        ...
        'focus-visible-ring',
        // estado
        ariaDisabled ? 'opacity-60 cursor-not-allowed' : 'interactive',
        className,
      ].join(' ')}
    >
      ...
      >
        <span className="h-0 w-0 rounded-full bg-gold transition-all duration-500 ease-out group-hover:h-56 group-hover:w-56 motion-reduce:transition-none" />
      </span>
      {/* Brilho/gradiente sutil por cima para relevo */}
      <span aria-hidden className="pointer-events-none absolute inset-0 -z-0 rounded-lg bg-gradient-to-b from-white/0 via-white/0 to-black/10" />
      {/* Borda e cor de base (para estado não-hover) */}
      <span aria-hidden className="absolute inset-0 -z-20 rounded-lg bg-gold/90" />
